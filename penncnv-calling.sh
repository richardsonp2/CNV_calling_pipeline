...
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)" # absolute path of directory

# Gather input files
PENN_CNV_FILES_DIR="${SCRIPT_DIR}/penn_cnv_files"


# TODO check if there is more than one of these, throw an error

INPUT_FILE=$1
PFB_FILES=("$PENN_CNV_FILES_DIR"/*.pfb)

if (( ${#PFB_FILES[@]} == 0)); then
  echo "ERROR: No .pfb file found in $PENN_CNV_FILES_DIR" >&2
  exit 1

elif (( ${#PFB_FILES[@]} < 1)); then
  echo "ERROR: There is more than one pfb file found in $PENN_CNV_FILES_DIR" >&2
  printf '  %s\n' "${PFB_FILES[@]}" >&2
  exit 1
fi 

GCM_FILES=("$PENN_CNV_FILES_DIR"/*.gcmodel)

if (( ${#GCM_FILES[@]} == 0)); then
  echo "ERROR: No .gcmodel file found in $PENN_CNV_FILES_DIR" >&2
  exit 1

elif (( ${#GCM_FILES[@]} < 1)); then
  echo "ERROR: There is more than one gcmodel file found in $PENN_CNV_FILES_DIR" >&2
  printf '  %s\n' "${GCM_FILES[@]}" >&2
  exit 1
fi 
...
if (( ${#MASTER_LIST[@]} == 0)); then
  echo "ERROR: No .txt file found in $PENN_CNV_FILES_DIR" >&2
  exit 1

elif (( ${#MASTER_LIST[@]} < 1)); then
  echo "ERROR: There is more than one master list text file found in $PENN_CNV_FILES_DIR" >&2
  printf '  %s\n' "${MASTER_LIST[@]}" >&2
  exit 1
fi 
